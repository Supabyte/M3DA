...
int main(int argc, char **argv) {
    ...
    int iWindowHalfSize = 4; // Demi-taille de la fenetre de correlation
    ...
    if(bDisplay) {
        ...
        normalize(mLeftDisparity, mLeftDisparity, 0, 255, CV_MINMAX);
        minMaxLoc(mRightDisparity, &dMin, &dMax);
        if (bVerbose) {
            printf("Disparite maximale (ref droite) = %lf\n", dMax);
        }
        normalize(mRightDisparity, mRightDisparity, 0, 255, CV_MINMAX);
